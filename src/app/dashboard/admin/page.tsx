...
function AdminDashboardContent() {
  ...
  const activeTab =
    tabParam &&
    ["overview", "add-course-request", "approved-course", "users"].includes(
      tabParam
    )
      ? tabParam
      : "overview";
  ...
  const statsList = [
    ...
    },
    {
      ...
      loading: statsLoading,
      error: statsError,
      onClick: () => handleTabChange("add-course-request"),
    },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex p-1 bg-slate-100 dark:bg-zinc-800 rounded-xl w-fit overflow-x-auto no-scrollbar border border-slate-200 dark:border-zinc-700">
          {(
            ...
            const count =
              tab === "add-course-request"
                ? Array.isArray(courses)
                  ? courses.filter((c: Course) => c.status === "pending").length
                  : 0
                : tab === "approved-course"
                ? Array.isArray(courses)
                  ? courses.filter((c: Course) => c.status === "approved")
                      .length
                  : 0
                : tab === "users"
                ? Array.isArray(users)
                  ...
                  {coursesLoading ? (
                    <div className="space-y-3">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="h-12 bg-slate-100 dark:bg-zinc-800 rounded-lg animate-pulse"
                        />
                      ))}
                    </div>
                  ) : coursesError ? (
                    ...
                    (Array.isArray(courses)
                      ? courses
                          .filter((c: Course) => c.status === "pending")
                          .slice(0, 3)
                      : []
                    ).map((course: Course) => (
                      <div
                        key={course._id}
                        className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors"
                      >
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                    {usersLoading ? (
                      ...
                      <tr>
                        ...
                        >
                          <p>
                            No users match &quot;<strong>{userSearch}</strong>
                            &quot;
                          </p>
                        </td>
                      </tr>
                          ...
                          >
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <div className="h-8 w-8 rounded-full bg-violet-100 flex items-center justify-center overflow-hidden relative">
                                  {user.photoURL ? (
                                    <Image
                                      src={user.photoURL}
                                      alt={user.name || "User"}
...
interface CourseListProps {
  ...
  isError: boolean;
  onUpdate: {
    mutate: (variables: { courseId: string; status: string }) => void;
    isPending: boolean;
  };
  showActions: boolean;
  filterStatus: string;
            ...
            {isLoading ? (
              ...
              <tr>
                <td
                  ...
                  className="px-6 py-8 text-center text-slate-500"
                >
                  No courses match your search or status.
                </td>
              </tr>
            ) : (
              filtered.map((course: Course) => (
                <React.Fragment key={course._id}>
                  <tr
                    className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-colors group cursor-pointer"
                    onClick={() =>
                      setExpandedId(
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-lg bg-slate-100 overflow-hidden flex-shrink-0 relative">
                          <Image
                            src={
                              course.thumbnailUrl ||
                              "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=100&q=60"
                            }
                            fill
                            sizes="40px"
                            className="object-cover"
                      ...
                      <div className="flex items-center gap-2">
                        <div className="h-6 w-6 rounded-full bg-slate-100 overflow-hidden relative">
                          {course.currentOwner?.photoURL && (
                            <Image
                              src={course.currentOwner.photoURL}
                              fill
                              sizes="24px"
                              className="object-cover"
                              alt=""
                    ...
                    <td className="px-6 py-4">
                      <span className="text-xs px-2 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-400">
                        {course.category}
                      </span>
                    </td>
                    <td className="px-6 py-4">
...
function StatCard({
  ...
  return (
    <Card
      className={cn(
        "p-6 border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm relative overflow-hidden group",
