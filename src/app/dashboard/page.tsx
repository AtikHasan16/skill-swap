...
interface DashboardActivity {
  _id: string;
  fromUser: DashboardUser;
  toUser: DashboardUser;
  requestedCourse?: DashboardCourse;
...
export default function DashboardOverview() {
  ...
  const stats = [
    {
      title: "Owned Courses",
      value: statsData.ownedCourses.toString(),
      icon: BookOpen,
    ...
    },
    {
      ...
      value: statsData.totalExchanges.toString(),
      icon: ArrowLeftRight,
        ...
        {stats.map((stat, i) => {
          ...
          return (
            ...
            >
              <Card
                spotlight
                className="p-6 border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 backdrop-blur-sm"
              >
        ...
        >
          <Card className="h-full p-6 border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/30">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs font-semibold px-2 py-1 rounded-md bg-violet-100 text-violet-600 dark:bg-violet-900/30">
                Auto Update
              </span>
            </div>
            <div className="h-[300px] w-full bg-slate-50/50 dark:bg-zinc-800/50 rounded-2xl flex items-center justify-center border border-dashed border-slate-300 dark:border-zinc-700">
              <div className="text-center">
                ...
                <p className="text-slate-400 font-medium italic">
                  Detailed analytics will appear here as you trade more skills.
                </p>
              </div>
            </div>
          </Card>
              ...
              {recentActivity.length > 0 ? (
                recentActivity.map((activity: DashboardActivity) => {
                  ...
                  return (
                    <div
                      key={activity._id}
                      className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-100 dark:border-zinc-700 hover:border-violet-300 dark:hover:border-violet-500/50 transition-all group"
                    >
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-slate-200 overflow-hidden ring-2 ring-white dark:ring-zinc-800 relative">
                          {otherUser?.photoURL ? (
                            <Image
                        ...
                        <div className="max-w-[150px]">
                          ...
                          <p className="text-[10px] sm:text-xs text-slate-500 line-clamp-1 italic">
                            {isIncoming
                              ? "Requested your"
                              : "Wants to swap for"}{" "}
                            &quot;{activity.requestedCourse?.title || "Course"}
                            &quot;
                          </p>
                        </div>
                      </div>
