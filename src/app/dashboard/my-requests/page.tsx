    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Manage your incoming and outgoing knowledge trades.
        </p>
      </div>
          ...
          {exchanges?.length === 0 ? (
            ...
            >
              ...
              <p className="text-slate-500 text-sm">
                When you send or receive exchange offers, they will appear here.
              </p>
            </motion.div>
          ) : (
            exchanges?.map((req: ExchangeRequest, i: number) => {
              const isIncoming = req.toUser?._id === userId;
              const courseIncoming = isIncoming
                ? req.offeredCourse
                : req.requestedCourse;
              const courseOutgoing = isIncoming
                ? req.requestedCourse
                : req.offeredCourse;
                        ...
                        <div className="flex flex-col lg:flex-row items-center gap-6">
                          ...
                          <div className="flex-1 w-full flex gap-4 bg-slate-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-slate-100 dark:border-zinc-800">
                            <div className="h-16 w-24 rounded overflow-hidden bg-slate-200 flex-shrink-0 relative">
                              <Image
                                src={
                                  ...
                                  "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=200&q=60"
                                }
                                fill
                                sizes="96px"
                            ...
                            <div className="min-w-0">
                              <div className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 mb-1 uppercase tracking-tight">
                                YOU RECEIVE
                              </div>
                              <h3 className="font-bold text-slate-900 dark:text-slate-50 truncate">
                                {courseIncoming?.title || "Unknown Course"}
                              </h3>
                              <p className="text-xs text-slate-500 truncate">
                                {courseIncoming?.platform} •{" "}
                          ...
                          <div className="flex-1 w-full flex gap-4 bg-slate-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-slate-100 dark:border-zinc-800">
                            ...
                            <div className="min-w-0">
                              <div className="text-[10px] font-bold text-violet-600 dark:text-violet-400 mb-1 uppercase tracking-tight">
                                YOU OFFER
                              </div>
                              <h3 className="font-bold text-slate-900 dark:text-slate-50 truncate">
                                {courseOutgoing?.title || "Unknown Course"}
                              </h3>
                              <p className="text-xs text-slate-500 truncate">
                                {courseOutgoing?.platform} • Me
                              </p>
                            </div>
                          </div>
                        </div>
