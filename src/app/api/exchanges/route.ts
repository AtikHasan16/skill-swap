...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log("Creating exchange process started:", {
      fromUserId,
      requestedCourseId,
      offeredCourseId,
    });
    ...
    if (!requestedCourseId || !offeredCourseId) {
      return NextResponse.json(
        { error: "Both course IDs are required" },
        { status: 400 }
      );
    }

    if (
      !mongoose.Types.ObjectId.isValid(requestedCourseId) ||
      !mongoose.Types.ObjectId.isValid(offeredCourseId)
    ) {
      return NextResponse.json(
    ...
    if (!requestedCourse) {
      return NextResponse.json(
        { error: "Requested course not found" },
        { status: 404 }
      );
    }
    ...
    if (!offeredCourse.currentOwner || !requestedCourse.currentOwner) {
      return NextResponse.json(
        { error: "Course owner data missing" },
        { status: 500 }
      );
    }
