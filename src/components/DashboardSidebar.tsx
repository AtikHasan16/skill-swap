    ...
    ...(isAdmin
      ? [
          { name: "Overview", href: "/dashboard", icon: LayoutDashboard },
          {
            ...
            href: "/dashboard/admin?tab=approved-course",
            icon: BookOpen,
          },
          {
            name: "Add Course Request",
            href: "/dashboard/admin?tab=add-course-request",
            icon: PlusCircle,
          },
    ...
    <>
      <div className="flex h-16 items-center px-6 border-b border-slate-100 dark:border-zinc-800 justify-between">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="lg:hidden p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg"
          >
            <X className="h-5 w-5" />
          </button>
        )}
      ...
      <div className="flex-1 flex flex-col gap-1 p-4 overflow-y-auto">
        ...
        {sidebarLinks.map((link) => {
          ...
          const linkTab = link.href.includes("tab=")
            ? link.href.split("tab=")[1]
            : null;
